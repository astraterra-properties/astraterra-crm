...
export default function RentListingsPage() {
  ...
  const fetchData = async () => {
    const token = localStorage.getItem('token');
    let url = '/api/rent-listings?limit=100';
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const url = current.id ? `/api/rent-listings/${current.id}` : '/api/rent-listings';
  ...
  const handleDelete = async (id: number) => {
    ...
    const token = localStorage.getItem('token');
    await fetch(`/api/rent-listings/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
    fetchData();
  };
      ...
      <div className="bg-white border-b px-6 py-4" style={{ borderColor: '#E5E7EB' }}>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold" style={{ color: '#131B2B' }}>Rent Listings</h1>
              <p className="text-xs" style={{ color: '#9CA3AF' }}>{filtered.length} listings</p>
            </div>
          </div>
          <button
            onClick={() => { setCurrent({ status: 'available', furnished: 'unfurnished', lease_term: '1 year' }); setShowModal(true); }}
      ...
      <div className="px-6 py-5">
        ...
        <div className="bg-white rounded-xl border p-4 mb-5 flex flex-wrap gap-3" style={{ borderColor: '#E5E7EB', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          ...
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="px-3.5 py-2.5 text-sm border rounded-lg" style={inputStyle}>
            ...
            {['apartment','villa','townhouse','penthouse','commercial','office','retail','studio'].map(t => <option key={t} value={t} className="capitalize">{t}</option>)}
          </select>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead>
                  <tr style={{ background: '#131B2B' }}>
                    {['Title', 'Type', 'Community', 'Beds', 'Annual Rent', 'Available From', 'Status', 'Portals', 'Actions'].map(h => (
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-3.5">
                          ...
                          {l.ejari_number && <p className="text-xs" style={{ color: '#9CA3AF' }}>EJARI: {l.ejari_number}</p>}
                        </td>
                        ...
                        <td className="px-5 py-3.5 text-sm font-medium" style={{ color: '#C9A96E' }}>
                          {l.annual_rent ? `AED ${l.annual_rent.toLocaleString()}/yr` : '—'}
                        </td>
                        <td className="px-5 py-3.5 text-sm" style={{ color: '#374151' }}>{l.available_from || 'Now'}</td>
                        <td className="px-5 py-3.5">
              ...
              {filtered.length === 0 && (
                <div className="text-center py-16" style={{ color: '#9CA3AF' }}>
                  ...
                  <p className="text-sm">No rent listings found</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="p-6 border-b" style={{ borderColor: '#E5E7EB' }}>
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold" style={{ color: '#131B2B' }}>{current.id ? 'Edit Listing' : 'New Rent Listing'}</h2>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={current.property_type || ''} onChange={e => setCurrent({ ...current, property_type: e.target.value })}
                    ...
                    {['apartment','villa','townhouse','penthouse','commercial','office','retail','studio'].map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: '#374151' }}>Annual Rent (AED)</label>
                  <input type="number" value={current.annual_rent || ''} onChange={e => setCurrent({ ...current, annual_rent: parseInt(e.target.value) || undefined })}
                    className={inputCls} style={inputStyle} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: '#374151' }}>Security Deposit (AED)</label>
                  <input type="number" value={current.security_deposit || ''} onChange={e => setCurrent({ ...current, security_deposit: parseInt(e.target.value) || undefined })}
                    className={inputCls} style={inputStyle} />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: '#374151' }}>Available From</label>
                  <input type="text" value={current.available_from || ''} onChange={e => setCurrent({ ...current, available_from: e.target.value })}
                    className={inputCls} style={inputStyle} placeholder="e.g. Immediately, March 2026" />
                </div>
                <div>
