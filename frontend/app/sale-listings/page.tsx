...
const STATUS_OPTIONS = ['available', 'reserved', 'sold', 'off_market'];
...
const emptyForm = {
  ...
  bedrooms: 0, bathrooms: 0, size_sqft: 0, price: 0, price_per_sqft: 0,
  floor: 0, total_floors: 0, view: '', furnished: 'unfurnished',
  owner_name: '', owner_contact: '', owner_email: '',
  permit_number: '', bayut_id: '', pf_id: '', dubizzle_id: '',
...
export default function SaleListingsPage() {
  ...
  const fetchListings = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/sale-listings?limit=500', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const openEdit = (l: Listing) => {
    ...
    setForm({
      ...
      size_sqft: l.size_sqft || 0, price: l.price || 0, price_per_sqft: l.price_per_sqft || 0,
      ...
      permit_number: l.permit_number || '', bayut_id: l.bayut_id || '', pf_id: l.pf_id || '',
      dubizzle_id: l.dubizzle_id || '', description: l.description || '',
      status: l.status || 'available', featured: l.featured || 0,
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editId ? `/api/sale-listings/${editId}` : '/api/sale-listings';
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await fetch(`/api/sale-listings/${deleteId}`, { method: 'DELETE', headers });
      setDeleteId(null);
      fetchListings();
  ...
  return (
    <div className="min-h-screen p-4 md:p-6" style={{ background: '#f8fafc' }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#0a1628' }}>Sale Listings</h1>
          <p className="text-sm text-gray-500 mt-1">Manage properties for sale</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: 'Sold', value: stats.sold, color: '#991b1b' },
        ].map(s => (
            ...
            return (
              <div key={l.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="p-4">
                  ...
                  <p className="text-lg font-bold mt-2" style={{ color: '#d4af37' }}>{formatAED(l.price)}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-y-auto p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8">
            <div className="flex items-center justify-between p-5 border-b" style={{ background: '#0a1628', borderRadius: '16px 16px 0 0' }}>
              <h2 className="text-lg font-bold text-white">{editId ? 'Edit Listing' : 'New Sale Listing'}</h2>
            ...
            <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div><label className="text-xs text-gray-500">Price (AED)</label><input type="number" value={form.price} onChange={e => setForm({ ...form, price: parseFloat(e.target.value) || 0 })} className={inputClass} /></div>
                  <div><label className="text-xs text-gray-500">Price/sqft</label><input type="number" value={form.price_per_sqft} onChange={e => setForm({ ...form, price_per_sqft: parseFloat(e.target.value) || 0 })} className={inputClass} /></div>
