...
router.get('/:id', async (req, res) => {
  try {
    const result = await query('SELECT * FROM communities WHERE id = ?', [req.params.id]);
...
router.delete('/:id', requireMinRole('admin'), async (req, res) => {
  try {
    const result = await query('DELETE FROM communities WHERE id = ? RETURNING id', [req.params.id]);
    if (!result.rows.length) return res.status(404).json({ error: 'Not found' });
